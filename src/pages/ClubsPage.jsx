      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 pt-12 pb-8">
          <div className="max-w-2xl mb-8">
            <span className="section-label mb-2 block"><Users className="w-3.5 h-3.5 inline mr-1" />{clubs.length} clubs</span>
            <h1 className="font-display font-bold text-4xl sm:text-5xl text-gray-900 dark:text-gray-50 leading-tight mb-2">
              Find your<br /><span className="text-gradient">community</span>
            </h1>
            <p className="text-gray-500 dark:text-gray-400">From hackathons to dance stages — there's a club for every passion.</p>
          </div>

          <div className="flex flex-col sm:flex-row gap-3 mb-5">
            <SearchBar value={search} onChange={setSearch} placeholder="Search by name, tag, or interest…" className="flex-1 max-w-md" />
            {activeTag && (
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 py-10">
        {error && (
          <div className="text-center py-8 text-sm text-gray-500 dark:text-gray-400">
            Using cached data. Connect Firebase for live updates.
          </div>
        )}
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {filtered.map((club, i) => (
                <motion.div key={club.id || club.slug} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04, duration: 0.4 }}>
                  <ClubCard club={club} />
                </motion.div>
              ))}
            </div>
