...
const AVAILABLE_TAGS = [
  ...
  'Literature', 'Performing Arts', 'Engineering', 'Writing', 'Art', 'Hackathon',
  'Open Source', 'Visual Media', 'Athletics', 'Health', 'Theatre', 'Debate',
];
...
export default function ClubEditor({ club, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error('Save Failed', err.message || 'Could not save club profile. Please try again.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          {form.logo && (
            <img src={form.logo} alt="logo preview" className="mt-2 w-12 h-12 rounded-2xl object-cover border border-petal-100 dark:border-grape-700" onError={e => { e.target.style.display = 'none'; }} />
          )}
        </div>
        <div>
          ...
          {form.banner && (
            <img src={form.banner} alt="banner preview" className="mt-2 w-full h-16 rounded-2xl object-cover border border-petal-100 dark:border-grape-700" onError={e => { e.target.style.display = 'none'; }} />
          )}
        </div>
      </div>
